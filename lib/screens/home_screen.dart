...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    // 🛡️ 보안: 앱 시작 시 사용자 프로필(역할, 이름)을 DB에서 로드
    _initUser();
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          Container(
            ...
            child: IconButton(
              tooltip: '로그아웃',
              icon: Icon(Icons.logout_rounded, color: isDark ? Colors.white70 : const Color(0xFF64748B), size: 20),
              onPressed: () async {
          ...
          builder: (context, dkSnapshot) {
            return StreamBuilder<List<BankTransaction>>(
              ...
              builder: (context, pendingSnapshot) {
                ...
                final isCW = auth.userRole == 'cw';
                final isDK = auth.userRole == 'dk';
    ...
    return StreamBuilder<List<BankTransaction>>(
      ...
      builder: (context, approvalsSnapshot) {
        final myPendingCount = approvalsSnapshot.data
                ?.where((tx) => tx.name == bankId) // 다시 ID(dk/cw)로 필터링하여 DB와 일치시킴
                .length ??
            0;
